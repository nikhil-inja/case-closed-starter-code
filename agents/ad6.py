...
# Add after your existing globals like TT_CACHE
KILLER_MOVES = {}  # Stores moves that caused cutoffs
MAX_KILLERS = 2     # Store top 2 killer moves per depth
...
def store_killer(depth, move, move_type='max'):
    """Store move that caused beta/alpha cutoff"""
    key = (depth, move_type)
    if key not in KILLER_MOVES:
        KILLER_MOVES[key] = []
    
    if move not in KILLER_MOVES[key]:
        KILLER_MOVES[key].insert(0, move)
        if len(KILLER_MOVES[key]) > MAX_KILLERS:
            KILLER_MOVES[key].pop()

def order_moves(moves, depth, move_type='max'):
    """Reorder to try killer moves first"""
    key = (depth, move_type)
    if key not in KILLER_MOVES:
        return moves
    
    killers = KILLER_MOVES[key]
    killer_moves = [m for m in moves if m in killers]
    other_moves = [m for m in moves if m not in killers]
    
    # Keep your boost ordering within each group
    killer_moves.sort(key=lambda m: m[1])
    other_moves.sort(key=lambda m: m[1])
...
def minimax_search(state, depth, is_max_turn, my_id, alpha, beta, weights):
    ...
    # ✅ CHANGE 1: Use killer move ordering
    max_moves = order_moves(max_moves, depth, move_type='max')
    min_moves = order_moves(min_moves, depth, move_type='min')
    ...
    for my_dir, my_boost in max_moves:
        ...
        for opp_dir, opp_boost in min_moves:
            ...
            # ✅ CHANGE 2: Store opponent's killer move
            if worst_case_score <= alpha:
                store_killer(depth, (opp_dir, opp_boost), move_type='min')
                break 
        ...
        # ✅ CHANGE 3: Store our killer move
        if alpha >= beta:
            store_killer(depth, (my_dir, my_boost), move_type='max')
            break
