...
WEIGHT_PROFILES = {
    ...
    "territorial_emergency": {
        "space": 3.0, "center": 0.0, "pressure": 0.0, "trapping": 0.5, 
        "openness": 2.0, "boost_preservation": 1.0
    }
}
...
def simulate_move_inplace(state, player_id, direction, use_boost):
    ...
    if direction == OPPOSITE_DIR.get(current_agent_dir):
        direction = current_agent_dir # The move defaults to the *current* direction
...
def get_best_move(state, my_id):
    ...
    if turn_count > 10 and turn_count % 10 == 0:
        # Run a quick Voronoi check *only for modeling*
        temp_board = state["board"]
        temp_my_head = state["agent1_trail"][-1]
        temp_opp_head = state["agent2_trail"][-1]
        my_v_space, opp_v_space, separated = calculate_voronoi_space(temp_board, temp_my_head, temp_opp_head)
        
        if separated and (opp_v_space > my_v_space * 1.5):
            CURRENT_OPPONENT_STRATEGY = "territorial_emergency"
            print("!!! STRATEGY: TERRITORIAL EMERGENCY")
        else:
            my_head = state["agent1_trail"][-1]; opp_head = state["agent2_trail"][-1]
            distance = calculate_opponent_distance(my_head, opp_head)
