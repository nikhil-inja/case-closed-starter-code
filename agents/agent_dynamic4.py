...
# ⚡ UPGRADE: We can now search much deeper.
MAX_SEARCH_DEPTH = 20
...
AGENT_NAME = "CaseClosed_SuperDemon_v4"

# --- All Heuristic Functions (Unchanged) ---
# (calculate_voronoi_space, flood_fill, evaluate_state, etc. are identical)
...
def evaluate_state(state, my_id, weights):
    ...
    if my_routes == 1 and my_space < 15: return -800000 + my_space * 1000
    if are_separated:
    ...
    if opp_space == 0 and my_space > 0 and count_escape_routes(board, opp_head) == 0:
        return 999999
    return final_score
...
# --- High-Speed Simulation & Search Logic (With Upgrades) ---
...
def undo_moves(state, undo_actions):
    # (Slight modification to handle player_id for undo)
...
def minimax_search(state, depth, is_max_turn, my_id, alpha, beta, weights):
    ...
    # ⚡ OPTIMIZATION 1: REMOVED `state["board"].tobytes()`
    # Hashing trails is faster and is the *true* source of state.
    state_hash = (
    ...
    cache_key = (state_hash, depth) # No longer need is_max_turn
    ...
    # (Rest of search logic is identical to your "UndoBot")
    max_id = 1; min_id = 2
...
def get_best_move(state, my_id):
    ...
    # ⚡ OPTIMIZATION 2: Initialize Move Ordering
    # Start with non-boost moves first
    my_moves.sort(key=lambda m: m[1])
    # This dict will store scores from one depth to guide the next
    ...
    for depth in range(2, MAX_SEARCH_DEPTH + 2, 2):
        ...
        # ⚡ OPTIMIZATION 2: Search best moves from last iteration *first*
        ...
        for move_dir, move_boost in sorted_moves:
            ...
            my_survived, my_undo = simulate_move_inplace(state_copy, 1, move_dir, move_boost)
            
            for opp_move_dir, opp_move_boost in opp_moves:
                opp_survived, opp_undo = simulate_move_inplace(state_copy, 2, opp_move_dir, opp_move_boost)
                ...
                if not my_survived and not opp_survived: score = 0
                    ...
                    score = minimax_search(state_copy, depth - 1, True, my_id, -np.inf, np.inf, selected_weights)
                
                state_copy["turn_count"] -= 1
                undo_moves(state_copy, opp_undo)
                worst_case_score = min(worst_case_score, score)
                if worst_case_score <= current_max_score_for_depth:
                   break
            
            # ⚡ OPTIMIZATION 2: Store score for this move
